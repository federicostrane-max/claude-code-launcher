"""
Claude Code Launcher - Multi-Project (Fixed)
...
def decode_project_path(folder_name):
    ...
    Il problema è che '-' può essere sia '\' che ' ' (spazio).
    Proviamo prima con il mapping salvato, poi con euristica.
    """
    ...
    # Formato: LETTERA--resto-del-percorso
    ...
    # Strategia: prova combinazioni comuni
    # Prima prova sostituendo gli ultimi segmenti con spazi
    parts = rest.split('-')
    
    # Prova a ricostruire il percorso testando varie combinazioni
    for i in range(len(parts)):
        # Prova: primi i segmenti con \, resto con spazi
        if i == 0:
    ...
    # Fallback: ritorna la versione con tutti backslash
...
def get_session_summary(jsonl_path):
    ...
    if data:
        # Cerca il primo messaggio utente come riassunto
...
def get_project_info(project_dir):
    ...
    # Cerca file .jsonl (sessioni) - solo nella root del progetto, non nelle sottocartelle
    ...
    # Ultima modifica = sessione più recente
    if info['sessions']:
...
def launch_claude(project_path, session_id=None):
    ...
    # Crea il comando - usa un file batch temporaneo per evitare problemi con le virgolette
...
'''
        ...
        # Esegui il batch in una nuova finestra
        subprocess.run(f'start cmd /k "{batch_path}"', shell=True)
    else:
    ...
    while True:
        ...
        try:
            ...
            if 1 <= idx <= len(projects):
                ...
                # Mostra le sessioni
                os.system('cls' if sys.platform == 'win32' else 'clear')
                proj['real_path'] = path  # Aggiorna con il percorso confermato
